...
public:
	Paginator(Iterator begin, Iterator end, size_t page_size) {
		...
		while (begin_ != end) {
			size_t range_size = end - begin_;
			cont_.push_back({begin_, next(begin_, min(page_size, range_size))});
			begin_ = next(begin_, min(page_size, range_size));
		}
	}
...
private:
	vector<IteratorRange<Iterator>> cont_;
};
